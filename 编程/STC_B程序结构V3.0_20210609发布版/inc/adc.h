...
enumAdcincEXT   :  含对扩展接口EXT设置ADC功能（EXT上P1.0、P1.1不可作数字IO功能使用）
enumAdcexpEXT   :  不含对扩展接口EXT设置ADC功能（EXT上P1.0、P1.1可作数字IO功能使用） 		
  (2)  GetAdcResult(char ADC_name)：获取指定通道ADC值。
函数参数：
          ADC_name：指定要获取ADC值的对象。取值：
						enumAdcXP10     ——  扩展接口上的 P1.0脚
						enumAdcXP11     ——  扩展接口上的 P1.1脚
						enumAdcRt        ——  热敏电阻
						enumAdcRop      ——  光敏电阻
						enumAdcNav      ——  导航按键
补充说明1：对于导航按键，下面GetAdcNavAct函数输出消抖后导航按键事件和状态，更方便使用；
补充说明2：当参数取值不在有效范围，函数将返回enumKeyFail
函数返回值：返回所查对象10bit AD值(电压）。
每个数字表示  VCC/1024（单位：V），其中VCC为电路板供电电压（USB接口一般为5V左右） 
...
编写：徐成（电话18008400450）   2021年3月25日设计
*/
...
#define _adc_H_
   ...
   extern unsigned int GetAdcResult(char ADC_name);
	 ...
	 enum KN_name  {enumAdcNavKey3=0,enumAdcNavKeyRight,enumAdcNavKeyDown,enumAdcNavKeyCenter,enumAdcNavKeyLeft,enumAdcNavKeyUp}; //导航按键名,(含K3)
#endif